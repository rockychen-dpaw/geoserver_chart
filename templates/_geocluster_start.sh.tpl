...
if [[ "${HOSTNAME}" == "{{ $.Release.Name }}-geocluster-0" ]]; then
  ...
  cp -f /geoserver/settings/admin.cluster.properties ${EXTRA_CONFIG_DIR}/cluster.properties
else
  echo "Copy the cluster.properties for geocluster slave to ${EXTRA_CONFIG_DIR}"
  cp -f /geoserver/settings/slave.cluster.properties ${EXTRA_CONFIG_DIR}/cluster.properties
fi
status=$((${status} + $?))
