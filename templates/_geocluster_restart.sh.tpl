{{- define "geocluster.restart" }}#!/bin/bash
...
{{- if eq ($.Values.geoserver.replicas | default 1 | int) 1 }}
#replicas is 1, restart now
...
if [[ "{{ $.Release.Name}}-geocluster-{{$i}}" != "${HOSTNAME}" ]] && [[ ${status} -eq 0 ]]; thena
...
fi #end for  if "${GEOCLUSTER_ROLE}" == "admin" 
