...
wget --timeout=0.5 http://127.0.0.1:8080/geoserver/web -o /dev/null -O /dev/null
status=$?
...
if [[ $status -eq 0 ]]; then
    ...
    sed -i "s/<ol>/<ol>\n<li><label>Start At:<\/label><span>${starttime}<\/span><label>Ready At:<\/label><span>${readytime}<\/span><label>Spent:<\/label><span>${startingtime}<\/span><label>Next Restart At:<\/label><span>${nextRestartTime}<\/span><\/li>/g" ${GEOSERVER_DATA_DIR}/www/server/starthistory.html
